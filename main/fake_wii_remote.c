...
void post_hid_request_reponse(uint16_t con_handle, HID_REPORT_PACKET* packet, uint16_t size)
{
    ...
    if (size == 23 && p[1] == 0x16)
    {
        printf("INFO : 0x16 read memory report\n");
...
void continous_reporting_task(void* p)
{
    ...
    for (;;)
    {
        ...
        else if (data_report_id == 0x33)
        {
            if(loop==5)
            {
                post_bt_packet(create_output_report_packet(wii_controller.wii_con_handle, wii_controller.data_cid, (uint8_t*)"\xa1\x33\x00\x80\x7f\x81\x9c\xff\x80\x55\xff\x80\xff\x80\x5f\xff\xff\xff\xff", 19));
                printf("WIIMOTE : a1 33 00 80 7f 81 9c ff ff ff ff ff ff ff ff ff ff ff ff ff ff ff ee\n");
                loop=0;
            }
            else
            {
                post_bt_packet(create_output_report_packet(wii_controller.wii_con_handle, wii_controller.data_cid, (uint8_t*)"\xa1\x33\x00\x80\x7f\x81\x9c\xff\x80\x55\xff\x80\xff\x80\x5f\xff\xff\xff\xff", 19));
                printf("WIIMOTE : a1 33 00 10 7f 81 9c ff ff ff ff ff ff ff ff ff ff ff ff ff ff ff ee\n");
                loop=loop+1;
            }
        }
